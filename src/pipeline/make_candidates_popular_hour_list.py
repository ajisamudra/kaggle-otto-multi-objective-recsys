...
from src.utils.date_function import get_hour_from_ts
...
def suggest_candidates(df: pd.DataFrame, event: str, name: str, tmp_cands_path: Path):
    ...
    # read candidate
    filepath = f"{tmp_cands_path}/{name}_popular_hour_{event}.parquet"
    cand_df = pd.read_parquet(filepath)

    # get session last ts and convert it to date
    df = df.groupby("session")["ts"].max().reset_index()
    df["hour"] = df["ts"].apply(lambda x: get_hour_from_ts(x))
    # add event in session
    df["session"] = df["session"].astype(str) + "_" + event

    # left join with cand_df
    df = df.merge(cand_df, how="left", left_on="hour", right_on="hour")
    # drop unused columns
    df = df.drop(columns=["ts", "hour"])

    return df


def generate_candidates_popular_hour(
    name: str,
    input_path: Path,
    tmp_cands_path: Path,
    output_path: Path,
    ...
    for ix in tqdm(range(n)):
        ...
        for event in ["clicks", "carts", "orders"]:
            ...
            filepath = output_path / f"{name}_{ix}_{event}_popular_hour_list.parquet"
...
def main(mode: str):
    ...
    if mode == "training_train":
        ...
        generate_candidates_popular_hour(
            name="train",
