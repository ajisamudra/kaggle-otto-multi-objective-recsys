...
def perform_eda(events: list, n: int):
    input_path = ""
    val_input_path = ""
    scoring_input_path = ""
    week = "w2"
    ...
    for EVENT in events:
        ...
        df = pd.concat([df, train_df, val_df, test_df])
        ...
        for feature in tqdm(df.select_dtypes(include=[int, float]).columns):
            if feature == TARGET:
                continue
            ...
            plt.savefig(f"{filepath}/{feature}_across_dataset.png", bbox_inches="tight")
        ...
        logging.info(f"EDA artifacts for event {EVENT} could be found here {filepath}")
...
def main(event: str = "all", n: int = 1):
    events = ["clicks", "carts", "orders"]
    if event != "all":
        events = [event]
    perform_eda(events=events, n=n)
