...
def scoring_treelite(artifact: str, event: str, week_data: str, week_model: str):
    # for each event
    ...
    for j, _ in enumerate(model.list_models):
        treelite_model_path = f"{artifact_path}/{artifact}/treelite_lgbm_model_{j}.so"
        predictor = treelite_runtime.Predictor(treelite_model_path, verbose=True)
