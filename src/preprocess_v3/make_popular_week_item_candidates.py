...
def gen_popular_week_item(data: pl.DataFrame):
    """
    df input
    session | type | ts | aid
    123 | 0 | 12313 | AID1
    ...
    popular_click = popular_click.with_columns([pl.lit("clicks").alias("type")])
    popular_cart = data_agg.filter(pl.col("cart_rank") <= 20)
    popular_cart = popular_cart.sort(by="cart_rank")
    popular_cart = popular_cart.with_columns([pl.lit("carts").alias("type")])
    popular_order = data_agg.filter(pl.col("order_rank") <= 20)
    popular_order = popular_order.sort(by="order_rank")
    popular_order = popular_order.with_columns([pl.lit("orders").alias("type")])

    # agg for candidate list
    popular_click = popular_click.groupby("type").agg(
        [
            pl.col("aid").list().alias("labels"),
            pl.col("click_rank").list().alias("ranks"),
        ]
    )
    popular_cart = popular_cart.groupby("type").agg(
        [
            pl.col("aid").list().alias("labels"),
            pl.col("cart_rank").list().alias("ranks"),
        ]
    )
    popular_order = popular_order.groupby("type").agg(
        [
            pl.col("aid").list().alias("labels"),
...
def make_popular_week_candidates(
    name: str,
    input_path: Path,
    output_path: Path,
):
    ...
    popular_click, popular_cart, popular_order = gen_popular_week_item(data=df)

    filepath = output_path / f"{name}_popular_week_clicks.parquet"
    ...
    filepath = output_path / f"{name}_popular_week_carts.parquet"
    logging.info(f"save chunk to: {filepath}")
    popular_cart.write_parquet(f"{filepath}")
    logging.info(f"output df shape {popular_cart.shape}")

    filepath = output_path / f"{name}_popular_week_orders.parquet"
    logging.info(f"save chunk to: {filepath}")
