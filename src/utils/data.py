...
def convert_top20_covisitation_click(mode: str = "local"):
    ...
    top_20_clicks = pqt_to_pair_dict(
        pd.read_parquet(covisit_dir / f"top_20_clicks_v{VER_FEA}_0.pqt")
    )
    for k in range(1, DISK_PIECES):
        top_20_clicks.update(
            pqt_to_pair_dict(
                pd.read_parquet(covisit_dir / f"top_20_clicks_v{VER_FEA}_{k}.pqt")
            )
        )

    filepath = f"{covisit_dir}/top_20_clicks_v{VER_FEA}.pkl"
...
def convert_top15_covisitation_buy2buy(mode: str = "local"):
    ...
    top_15_buy2buy = pqt_to_pair_dict(
        pd.read_parquet(covisit_dir / f"top_15_buy2buy_v{VER_FEA}_0.pqt")
    )

    filepath = f"{covisit_dir}/top_15_buy2buy_v{VER_FEA}.pkl"
...
def convert_top15_covisitation_buys(mode: str = "local"):
    ...
    top_15_buys = pqt_to_pair_dict(
        pd.read_parquet(covisit_dir / f"top_15_carts_orders_v{VER_FEA}_0.pqt")
    )
    for k in range(1, DISK_PIECES):
        top_15_buys.update(
            pqt_to_pair_dict(
                pd.read_parquet(covisit_dir / f"top_15_carts_orders_v{VER_FEA}_{k}.pqt")
            )
        )

    filepath = f"{covisit_dir}/top_15_carts_orders_v{VER_FEA}.pkl"
    joblib.dump(top_15_buys, filepath)


def get_top20_covisitation_click_dict(mode: str = "local") -> dict:
    logging.info(f"load ver {VER_FEA}")
