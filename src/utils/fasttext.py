...
def load_annoy_idx_fasttext_skipgram_embedding(mode: str = "local"):

    if mode == "local":
        emd_path = get_local_fasttext_dir()
        filepath = (
            f"{emd_path}/fasttext_local_skipgram_vec{VECTOR}_wdw{WINDOW}_neg{NEGATIVE}"
        )
    else:
        emd_path = get_scoring_fasttext_dir()
        filepath = f"{emd_path}/fasttext_scoring_skipgram_vec{VECTOR}_wdw{WINDOW}_neg{NEGATIVE}"
