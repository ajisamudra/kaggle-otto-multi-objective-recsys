...
def generate_candidates_word2vec(
    ...
    for ix in tqdm(range(n)):
        ...
        ses2aids = (
            df.groupby("session")["max_weighted_log_duration_event_in_session_aid"]
            .apply(list)
            .to_dict()
        )
        ...
        clicks_candidates_series, ranks_list = suggest_clicks_word2vec(
            n_candidate=CFG.word2vec_weighted_duration_candidates,
            ses2aids=ses2aids,
            embedding=embedding,
        )
        ...
        for event in ["clicks", "carts", "orders"]:
            ...
            filepath = (
                output_path
                / f"{name}_{ix}_{event}_word2vec_weighted_duration_list.parquet"
            )
