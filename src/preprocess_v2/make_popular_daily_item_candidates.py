...
def gen_popular_daily_item(data: pl.DataFrame):
    """
    df input
    session | type | ts | aid
    123 | 0 | 12313 | AID1
    ...
    # agg per date
    data_agg = data.groupby(["date", "aid"]).agg(
        [
            # num of event type
    ...
    popular_click = popular_click.groupby("date").agg(
        [pl.col("aid").list().alias("labels")]
    )
    popular_cart = popular_cart.groupby("date").agg(
        [pl.col("aid").list().alias("labels")]
    )
    popular_order = popular_order.groupby("date").agg(
        [pl.col("aid").list().alias("labels")]
    )
...
def make_popular_daily_candidates(
    name: str,
    input_path: Path,
    output_path: Path,
):
    ...
    popular_click, popular_cart, popular_order = gen_popular_daily_item(data=df)

    filepath = output_path / f"{name}_popular_daily_clicks.parquet"
    ...
    filepath = output_path / f"{name}_popular_daily_carts.parquet"
    logging.info(f"save chunk to: {filepath}")
    popular_cart.write_parquet(f"{filepath}")
    logging.info(f"output df shape {popular_cart.shape}")

    filepath = output_path / f"{name}_popular_daily_orders.parquet"
    logging.info(f"save chunk to: {filepath}")
